...
const SpeedometerChart: React.FC<SpeedometerChartProps> = ({
  ...
  const endAngle = startAngle + totalArcDegrees; // 495 degrees (+135 degrees)

  let currentAngle = startAngle;
  const chartConfigs: any[] = [];

  // Filter out segments with 0% before processing
  const filteredSegments = normalizedSegments.filter(segment => segment.percentage > 0);
  
  // Debug logging to see what segments are actually being processed
  console.log('📊 Chart Debug - centerLabel:', centerLabel);
  console.log('📊 Original Segments:', normalizedSegments);
  ...
  return (
    <div className={`relative group ${className}`} style={{ width: size, height: size }}>
      ...
      {centerValue !== '100' && (
        ...
        <span className="text-3xl font-bold text-gray-800 ml-1 tracking-tight">
          %
        </span>
        </div>
      )}
        ...
        <div className="text-center">
          ...
          >
            ...
            {centerSubLabel && (
              <>
                <br />
                <span className="text-sm text-gray-700 font-normal">{centerSubLabel}</span>
              </>
            )}
          </div>
        </div>
